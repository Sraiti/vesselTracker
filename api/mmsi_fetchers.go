...
// func getVesselMMSIbyIMO(imo string, database *sql.DB) (string, error) {

// 	///check db before we fetch it from the internet

// 	var mmsi string

// 	vessel, err := db.GetVesselByIMO(database, imo)

// 	if err == nil {
// 		log.Printf("Found MMSI for IMO %s in DB: %s", imo, vessel.MMSI)
// 		mmsi = vessel.MMSI
// 		return mmsi, nil
// 	}
// 	log.Printf("No MMSI found for IMO %s in DB", imo)
